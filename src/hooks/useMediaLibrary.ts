...
export function useMediaLibrary() {
  ...
  async function loadTracks(threshold = minDuration) {
    ...
    try {
      ...
      const initialTracks: Track[] = allAssets
        .filter((a) => a.uri && typeof a.duration === 'number' && a.duration >= threshold)
        .map((a) => ({
          id: a.id,
        ...
        await Promise.all(
          chunk.map(async (track) => {
            try {
              ...
              // Ignore corrupted ID3 tags or inaccessible files
              track.artUri = null;
            }
          })
        );

        // Trigger a re-render so UI picks up updated metadata (spread to force update)
        setTracks((prev) => [...prev]);
      }
    } catch (error) {
